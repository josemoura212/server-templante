...
impl EmailClientSettings {
    ...
    pub fn sender(&self) -> Result<EmailUser, String> {
        EmailUser::parse(self.sender_email.clone())
    }
...
pub fn get_configuration() -> Result<Settings, config::ConfigError> {
    impl TryFrom<config::Config> for Settings {
        ...
        fn try_from(config: config::Config) -> Result<Self, Self::Error> {
            ...
            let redis_uri = config.get::<SecretString>("redis_uri")?;
            let file_path = config.get::<String>("file_path")?;
